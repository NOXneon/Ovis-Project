...
public class WeekView extends View {
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            switch (mCurrentFlingDirection) {
                case LEFT:
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * (MAX_HOUR - MIN_HOUR +1) + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * (MAX_HOUR - MIN_HOUR +1) + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()), 0);
                    break;
            }
    ...
    private void initTextTimeWidth() {
        mTimeTextWidth = 0;
        for (int i = 0; i < (MAX_HOUR - MIN_HOUR)+1; i++) {
            // Measure time string and get max width.
            String time = getDateTimeInterpreter().interpretTime(i + MIN_HOUR);
            if (time == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null time");
            mTimeTextWidth = Math.max(mTimeTextWidth, mTimeTextPaint.measureText(time + MIN_HOUR));
        }
    }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * (MAX_HOUR - MIN_HOUR +1) - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2)
            mCurrentOrigin.y = getHeight() - mHourHeight * (MAX_HOUR - MIN_HOUR +1) - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 0;
            for (int hourNumber = 0; hourNumber < (MAX_HOUR - MIN_HOUR)+1; hourNumber++) {
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    float top = mHourHeight * (MAX_HOUR - MIN_HOUR+1) * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;
                    ...
                    bottom = mHourHeight * (MAX_HOUR - MIN_HOUR+1) * bottom / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;
