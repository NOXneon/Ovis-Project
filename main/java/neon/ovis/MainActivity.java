...
public class MainActivity extends Activity
    ...
    private class MakeRequestTask extends AsyncTask<Void, Void, List<String>> {
        ...
        private com.google.api.services.calendar.model.Calendar ovis = null;
        ...
        protected List<String> doInBackground(Void... params) {
            try {
                ...
                try
                {
                    ovis = mService.calendars().get("Ovis").execute();
                }
                catch (Exception e)
                {
                    e.printStackTrace();
                    ...
                    c.setTimeZone(gmt);
                    try {
                        ovis = mService.calendars().insert(c).execute();
                ...
                {
                    l = lines.get(i);
                    insertEvent(l.getSubject(), l.getLocation(), l.getDescription(), l.sDateTime(), l.eDateTime(), null);
                }
                */

                Calendar cal = Calendar.getInstance();
                cal.setTimeInMillis(0);
                cal.set(2018, 6-1,20, 11, 17, 00);
                ...
                try {
                    insertEvent("Subject test", "Location test", "Description test", new DateTime(startDate), new DateTime(endDate), null);
                } catch (IOException e) {
                    e.printStackTrace();
                }
        ...
        void insertEvent(String summary, String location, String des, DateTime startDate, DateTime endDate, EventAttendee[] eventAttendees) throws IOException
        {
            ...
            event.setRecurrence(Arrays.asList(recurrence));     if(eventAttendees != null) event.setAttendees(Arrays.asList(eventAttendees));     EventReminder[] reminderOverrides = new EventReminder[] {
                    new EventReminder().setMethod("popup").setMinutes(10),
            };
            Event.Reminders reminders = new Event.Reminders()
                    .setUseDefault(false)
                    .setOverrides(Arrays.asList(reminderOverrides));
            event.setReminders(reminders);     String calendarId = ovis.getId();
            //event.send
            if(mService!=null)
